...
describe('events create', () => {
  ...
  it('accepts attendees flag', async () => {
    try {
      await runCommand('events create "Test Event" --start "2024-01-15T14:00:00" --attendees "alice@example.com,bob@example.com"')
    } catch (error) {
      // Command should parse flags correctly even if authentication fails
      expect(String(error)).to.not.contain('Unknown flag')
    }
  ...
  it('uses short flag aliases', async () => {
    try {
      await runCommand('events create "Test Event" -s "2024-01-15T14:00:00" -e "2024-01-15T15:00:00" -c primary -l "Room A" -f json')
    } catch (error) {
